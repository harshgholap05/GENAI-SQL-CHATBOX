...
|------|----------|-------------|
| 1 | `clean_sql_output` | Strips markdown fences, extracts pure SQL |
...
└── frontend/
    ├── src/
    │   ├── App.jsx            # Main app — chat, JOIN badge, chart suggestion
    │   ├── index.css          # Global styles + dark mode
    │   ├── main.jsx
    │   └── components/
    │       ├── Sidebar.jsx    # Chat history, delete, clear all
    │       ├── Chat.jsx
    │       ├── LoadTable.jsx
    │       └── TableInfo.jsx
    ├── package.json
...
3. Meta/DB questions handled separately without SQL
4. Smart multitable detection runs — if question spans multiple tables, auto-JOIN chain built
5. Otherwise single-table SQL generated → cleaned → validated → executed
6. Result returned with explanation + optional chart suggestion

### Mode 2 — No Table Selected (Full DB Mode)
1. All tables + all FK relationships fetched dynamically
2. Full schema + FK map passed to LLM as context
...
1. User types: `join Orders to Customers`
